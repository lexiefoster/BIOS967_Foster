...
#### DS: I think the reason not all of your correlations are showing is because some of your columns have NAs.

Ths shows how many NAs are in each variable:
```{r}
sapply(dat2, function(x) length(which(is.na(x))))
```

If you run the ggcorrplot() function while dropping the NAs, you get clearer results:
```{r}
ggcorrplot(cor(dat2 %>% drop_na()))
```
